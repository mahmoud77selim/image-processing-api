import fs from 'fs';
import { imagesPath, clearnFiles,  createThumbnailName, imageDirsTypes} from '../utils';
interface checkImageReturnTypes { unResized: string[];inputFiles: string[]; outputFiles: string[];}

export const checkIfImagesExist = (width: number | null, height: number | null
  ...
  inputFiles.forEach((file) => {
    ...
    if (!outputFiles.includes(thumbnailFile)) {
      unResized.push(file);}});
  return { unResized,inputFiles,outputFiles};};
